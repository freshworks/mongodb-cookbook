...
Chef::Log.info("yum repo selected is #{node['mongodb']['repo']}/#{default[:mongodb][:package_base_version]}/#{node['kernel']['machine']  =~ /x86_64/ ? 'x86_64' : 'i686'}")
...
when 'rhel', 'fedora'
  yum_repository 'mongodb' do
    ...
    baseurl "#{node['mongodb']['repo']}/#{default[:mongodb][:package_base_version]}/#{node['kernel']['machine']  =~ /x86_64/ ? 'x86_64' : 'i686'}"
    action :create
    gpgcheck false
    enabled true
  end
